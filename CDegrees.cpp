...
void CDegrees::GameOver() {// Implement game over logic and winner determination
    std::cout << "\n\nGame Over" << std::endl;
    ...
    for (const auto& currentPlayer : mPlayers) {
        if(currentPlayer.HasGraduated()){
            std::cout << currentPlayer.GetName() << " has graduated and wins with " << currentPlayer.GetSuccess() << " achievements!" << std::endl;
            return;
        }
    }
